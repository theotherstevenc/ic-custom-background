...
const mockApiData = [ 
  'Bungee+Spice', 'Roboto', 'Ubuntu', 'Nabla', 'Open+Sans'
]
document.getElementById('external').setAttribute(
  'href', `https://fonts.googleapis.com/css?family=${mockApiData.map(el => el).join('|')}`
)
  ...
  mockApiData.forEach(element => {
    markup += `
      <span class="slds-radio">
        <input type="radio" value="${element}" id="${element}" name="ic-custom" />
        <label class="slds-radio__label" for="${element}" style="font-family: ${element.replace('+',' ')}; font-size: 2em;">
          <span class="slds-radio_faux"></span>
          <span class="slds-form-element__label">${element.replace('+',' ')}</span>
        </label>
      </span>
    `    
  });
...
function _display() {
  ...
  if(document.querySelector('input[name="ic-custom"]:checked')) { 
    googleFontFamily = document.querySelector('input[name="ic-custom"]:checked').value.replace('+',' ')
    ...
    <div style="display:table; width:100%; max-width:${bgWidth}px; background-image:url('${bgUrl}'); background-size: cover; text-align: center; color: ${bgTextColor};">
      ...
      <div style="display:inline-block; width:100%; ${googleFontStack} font-size:22px; background:${bgTextBgColor};">${bgText}</div>
    </div>
    `
  );
...
sdk.getData((data) => {
	...
	bgText = data.bgText || 'think locally'
